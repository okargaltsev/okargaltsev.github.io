...
$(document).ready(function () {
    ...
    $.getJSON('webdata/web.json', function (jsn_new) {
		
		var jsn = jsn_new.obsids;
		
		var debug_mode = jsn_new.debug_mode;
        ...
        for (var id in jsn) {
            ...
            ht += `<tr class="click ${cls}" id=${id}><td>` +
                ...
                `<a href=${jsn[id].url} download>evt2</a>`].join('</td><td>') +
                '</td></tr>';
        ...
        $('.click').click(function (event) {
            ...
            if ($r.hasClass('blank')) {
                ...
                var ht = '<div class="container kkk">';
                ...
                if (laslen > 0) {
                    laskeys.forEach(function (value, index, array) {
						...
						var poly_class = `polygon_green`;
						
						if (debug_mode) 						
							if (!(jsn[id].lassos[value].is_good)) 								
                                poly_class = `polygon_red`;   
                                						
                            if (jsn[id].lassos[value].shape == 'convex_hull') 								
                                poly_class = `polygon_yellow`;           
						
                        ht +=  `<polygon class="${id} ${value} ${poly_class}" points="`;
                        ...
                        smalltable[value] = `<div class="topleft" id="${id + '_' + value}"><table>
                                            ...
                                            <tr><td>S/N ratio</td><td>${pd.SN}</td></tr>`;

                        if  (debug_mode) 
                            smalltable[value] += `<tr><td>roundness</td><td>${pd.roundness}</td></tr>
                                                <tr><td>area</td><td>${pd.area}</td></tr>
                                                <tr><td>area_psf</td><td>${pd.area_psf}</td></tr>
                                                <tr><td>area_px</td><td>${pd.area_px}</td></tr>
                                                <tr><td>is_good</td><td>${pd.is_good}</td></tr>
                                                <tr><td>centrality</td><td>${pd.centrality}</td></tr>
                                                <tr><td>kurtosis</td><td>${pd.kurtosis}</td></tr>
                                                <tr><td>ks</td><td>${pd.ks}</td></tr>
                                                <tr><td>kuiper</td><td>${pd.kuiper}</td></tr>`;   

                                                
                        smalltable[value] += `</table></div>`;
            ...
            $("polygon").click(function (event) {
                ...
                $(this).parents(".kkk").append(smalltable[cls[1]]);
